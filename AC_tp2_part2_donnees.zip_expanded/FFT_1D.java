...
public class FFT_1D {
	...
	public static CpxTab combine(CpxTab c1, CpxTab c2) {
		...
		//A FAIRE taille c1="+c1.taille()+" taille c2="+c2.taille();
		int n = c1.taille()*2;
		CpxTab c = new CpxTab(n);
		for(int k=0;k<n/2;k++) {
			double re_gauche = c1.get_p_reel(k) + cos(-2*PI*k/n)*c2.get_p_reel(k) - sin(-2*PI*k/n)*c2.get_p_imag(k);
			double im_gauche = c1.get_p_imag(k) + sin(-2*PI*k/n)*c2.get_p_reel(k) + cos(-2*PI*k/n)*c2.get_p_imag(k);
			c.set_p_reel(k, re_gauche);
			c.set_p_imag(k, im_gauche);
			double re_droite = c1.get_p_reel(k) - cos(-2*PI*k/n)*c2.get_p_reel(k) + sin(-2*PI*k/n)*c2.get_p_imag(k);
			double im_droite = c1.get_p_imag(k) - sin(-2*PI*k/n)*c2.get_p_reel(k) - cos(-2*PI*k/n)*c2.get_p_imag(k);
			c.set_p_reel(k+n/2, re_droite);
			c.set_p_imag(k+n/2, im_droite);
		}
		return c;
	}
	...
	public static CpxTab FFT_inverse(CpxTab y) {
		//A FAIRE
		CpxTab coef = new CpxTab(y.taille());
		//System.out.println("affichage du coef : " + coef.toString());
		//System.out.println("taille du cpxtab pris en entrée : " + y.taille());
		//System.out.println("valeur d'un coef : " + (double)1/y.taille());
		for (int i=0; i<y.taille(); i++) {
			coef.set_p_reel(i,(double)1/y.taille());
		}
		CpxTab tfd = FFT(y.conjugue()).conjugue();
		//System.out.println("affichage de la tfd : " + tfd.toString());
		return CpxTab.multiplie(coef, tfd);
	}
	...
	public static void main(String[] args) {
		...
		//A FAIRE
		System.out.println(FFT(t5));

		/* Exo 3: calculez et affichez TFD_inverse(TFD(1,2,3,4)) */
		//A FAIRE
		System.out.println(FFT_inverse(FFT(t5)));
...
// Exercice 5 - Comparaison temps d'execution
        
        int []tableau = {2,4,8,16,32,64,128,256,512,1024,2048,4096,8192,16384,32768};
        List<Integer> mult_coeff = new ArrayList<Integer>();
        List<Integer> mult_fft = new ArrayList<Integer>();
        List<Integer> tableau_indice = new ArrayList<Integer>();
        for(int i=0;i<tableau.length;i++) {
            int taille= tableau[i];
            double[] tab3 =random(taille),tab4 = random(taille);
            long date3, date4;
            //calculs pour courbe via coeff
            date3 = System.currentTimeMillis();
            multiplication_polynome_viaCoeff(tab3, tab4);
            date4 = System.currentTimeMillis();
            mult_coeff.add((int)(date4-date3));
            //calculs pour courbe via fft
            date3 = System.currentTimeMillis();
            multiplication_polynome_viaFFT(tab3, tab4);
            date4 = System.currentTimeMillis();
            mult_fft.add((int)(date4-date3));
            
            tableau_indice.add(taille);
            
        }
        Graph g = new Graph(tableau_indice,mult_fft);
        g.display();
        System.out.println("AFFICHAGE");
        System.out.println("COEFF : "+mult_coeff);
        System.out.println("FFT : "+mult_fft);
	}
